...
| 8 | AI generates usable Macedonian lessons inside the БРО programs and the e-учебници | Teacher rates ≥ 4/5 on 10 generated lessons without heavy edits | Build the generator; test with the advising teacher | Dev | Untested |
...
| 10 | School wifi failure does not kill the lesson | Students on mobile data works in a concrete building; queued answers sync correctly | Free live session in a Bitola school | Team | Untested |
...
- **Practical lessons are out of v1.** In a technical school a large share of periods are
  практична настава, so lessons-per-teacher — the retention metric in
  [[channels-and-relationships]] — is structurally halved. Measure per *theory* period or the
  pilot will look worse than it is.
...
- **Points gaming.** Students will share answers. Shuffled option order, the rotating QR,
  one active session per school-email account and the speed bonus limit it; the teacher's
  discretion on grades removes the stakes that would make it worth the effort.
- **Support at 08:05.** Teachers sign in with an email and a password, so the
  forgotten-password call is guaranteed — make reset fast and phone-friendly, and lean on
  the champion teacher as first line.
- **Data incident.** EU hosting, controller/processor structure, and no standing staff
  access. The blast radius is names, attendance, points and per-topic results — larger than
  it was before per-student data, which is why the access log and the ДПА clauses matter.
